...
Then, in that same file, implement a function called main that prompts the user for input, calls convert on that input, and prints
the result. You’re welcome, but not required, to prompt the user explicitly, as by passing a str of your own as an argument to input.
Be sure to call main at the bottom of your file.
"""

smiley = input('Write something using :) and :(').replace(':)', '🙂').replace(':(', '🙁')
print(smiley)


#Verwendung einer Main Variable
def main():
    smiley = input('Write something using :) and :(').replace(':)', '🙂').replace(':(', '🙁')
    print(smiley)
